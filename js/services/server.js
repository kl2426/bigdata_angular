...
//			console.log(tree[i],tree[i].children)
	...
	this.outTree = function(det){
		if(det.length < 1)return [{label:'全部菜单',fucecode:"0",children:[],level:0}];
		//
		var tree = {};
		//
		for(var a in det){
		...
		//
		var data_tree = [{label:'全部菜单',fucecode:"0",children:tree[0].children,level:0}];
	...
	//  已知圆角度 半径 返回坐标
	this.Rotate = function(x0,y0,r,a0)
	{
		x1   =   x0   +   r   *   Math.cos(a0   *   Math.PI   /180   ) 
		y1   =   y0   +   r   *   Math.sin(a0   *   Math.PI   /180   ) 
	    return {
	        X : x1,
	        Y : y1
	    }
