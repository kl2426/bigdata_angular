 ...
 * 弹窗拖动
 */
angular.module('app').directive('uiEcharts', ['$timeout','$document', function($timeout,$document) {
    function link(scope, element, attr) {
		var ec_scope = attr.option;
		var ec_option = scope[ec_scope];
    	
    	scope.echarts_init = function(){
    		
    		var e_par = $(element[0]).parent();
    		$(element[0]).width(e_par.width());
    		
    		// 基于准备好的dom，初始化echarts实例
	        var myChart = echarts.init(element[0]);
	        // 使用刚指定的配置项和数据显示图表。
	        ec_option && myChart.setOption(ec_option);
    	}
